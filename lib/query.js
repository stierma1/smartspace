...
Query.prototype.getPayload = function(ruleFullId){
  ...
  var results = inCtxRulesContents.map(function(val){
    if(!!val.provider.provider){
      return self.facts.getPayload(val.provider.provider, val.predicate);
    }
    else if(val.provider && val.provider.class && self.classes.indexOf("class/" + val.provider.class) !== -1){
      return self.facts.getPayload(self.provider, val.predicate);
    }
    else if(val.provider.quantifier === "exists" && !!val.provider.class){
      var matched = self.graph.getNodeByEdge("class/" + val.provider.class, "classifies");
      for(var i = 0; i < matched.length; i++){
        if(self.facts.matchExists(removeTypeLabel(matched[i]), val.predicate, val.groundings)){
          return self.facts.getPayload(removeTypeLabel(matched[i]), val.predicate);
        }
      }
      return undefined; //No provider matched
    }
    else if(val.provider.quantifier === "all" && !!val.provider.class){
      var matched = self.graph.getNodeByEdge("class/" + val.provider.class, "classifies");
