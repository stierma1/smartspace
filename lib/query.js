...
Query.prototype.hasRuleChanged = function(satRules){
...
Query.prototype.hasRuleValueChanged = function(ruleId){
  var self = this;
  var classes = this.graph.getNodeByEdge("provider/" + this.provider, "is_a");
  ...
  var providers = ctxRulesContents.filter(function(val){
    ...
    return self.graph.getNodeByEdge(val.id, "satisfied");
  });

  var classThis = ctxRulesContents.filter(function(val){
    if(val.provider.quantifier === "invoker"){
...
Query.prototype.isRuleSatisfied = function(ruleId, classes){
  ...
  var classThis = ctxRulesContents.filter(function(val){
    if(val.provider.quantifier === "invoker"){
      ...
      winston.log("debug", "Invoker %s is not a member of %s", self.provider, val.provider.class, self._logObj);
    }
    return false;
  });
